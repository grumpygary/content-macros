const GrelMacros = require("../src/grel-macros");
...
let grelMacros = new GrelMacros(macroConfig);

describe("GrelMacros", () => {
    // let books, movies, quotes, bookId, movieId, quoteId;
    test("FUNC-1", async () => {
        let value = grelMacros.expand("{{func1,aaa,bbb}}");
        expect(value).toBe('ONE:aaa,bbb');
    });
    test("FUNC-2", async () => {
        let value = grelMacros.expand("{{=func2,aaa,bbb}}");
    ...
    test("NESTED", async () => {
        let value = grelMacros.expand("{{{{func}}{{string1}},ccc,ddd}}");
